...
# --------------------------------------------------------------------#
    ...
    #ctq_df['chrpps_protect']   = 6 - ctq_df['chrpps_protect'].astype(float)
    #ctq_df['chrpps_docr']      = 6 - ctq_df['chrpps_docr'].astype(float)
    ctq_final_df = create_total_division('ctq_trauma_general', ctq_df, df_all, ['chrpps_lazy', 'chrpps_born', 'chrpps_hate', 'chrpps_hurt',\
                                                                                ...
                                                                                'chrpps_dirty', 'chrpps_docr'], 1, voi_2, all_visits_list, 'float')
    ctq_final_df = create_total_division('ctq_physical_abuse', ctq_df, df_all, ['chrpps_doc', 'chrpps_bruise', 'chrpps_belt',\
                                                                                'chrpps_physab', 'chrpps_beat'], 1, voi_2, all_visits_list, 'float')
    ctq_final_df = create_total_division('ctq_sexual_abuse', ctq_df, df_all, ['chrpps_touch', 'chrpps_sexual', 'chrpps_molest',\
                                                                              'chrpps_sexab', 'chrpps_threat'], 1, voi_2, all_visits_list, 'float')
    ctq_final_df = create_total_division('ctq_emotional_abuse', ctq_df, df_all, ['chrpps_hurt', 'chrpps_hate', 'chrpps_emoab',\
                                                                                 'chrpps_lazy', 'chrpps_born'], 1, voi_2, all_visits_list, 'float')
    ctq_final_df = create_total_division('ctq_emotional_neglect', ctq_df, df_all, ['chrpps_special', 'chrpps_loved', 'chrpps_care',\
                                                                                   'chrpps_closefam', 'chrpps_support'], 1, voi_2, all_visits_list, 'float')
    ctq_final_df = create_total_division('ctq_physical_neglect', ctq_df, df_all, ['chrpps_hunger', 'chrpps_pardrunk', 'chrpps_dirty',\
...
# --------------------------------------------------------------------#
    ...
    if version == 'run_outcome' or version == 'single_subject':
        print('In this version we do not save the outcomes in the subject folders')
    ...
    output_df_id = pd.concat([ctq_final_df],\
                   axis = 0, sort = True)
    output_df_id['ID'] = id 
...
if version == 'test':
    ...
    concatenated_df.to_csv("/data/predict1/home/np487/amp_scz/test_subjects/{0}_add_on_new_subjects.csv".format(network), index = False, header=True, float_format='%.3f')
elif version == 'create_control':
    concatenated_df.to_csv("/data/predict1/home/np487/amp_scz/{0}_add_on_test_subjects.csv".format(network), index = False, header=True, float_format='%.3f')
elif version == 'run_outcome':
    print("Wrote the real outcome - subjects to the control_subjects folder.")
    concatenated_df.to_csv("/data/predict1/home/np487/control_subjects/{0}_add_on_calcs.csv".format(network), index = False, header=True, float_format='%.3f')
    # To know when the outcome_calculation script was run the last time for all subjects create a text file:
    current_date = datetime.now()
    # Format the date as a string
    formatted_date = current_date.strftime('%Y-%m-%d %H:%M:%S')
    # Create the message
    message = f'{Network}: only add-on preliminary outcome calculations were run. They will not update the real outcomes. The add-on calculations are run for the last time on: {formatted_date}'
    ...
    formatted_date = current_date.strftime('%Y-%m-%d %H:%M:%S')
    # Create the message
    message = f'Single Subject {id_list} outcome for {Network} only add-on preliminary outcome calculations were run. They will not update the real outcomes. The add-on calculations were run for the last time on: {formatted_date}'
    file_path = '/data/predict1/home/np487/amp_scz/outcome_calculations/last_date_runoutcome.txt'
    # read the file from before:
    existing_content = ''
    try:
